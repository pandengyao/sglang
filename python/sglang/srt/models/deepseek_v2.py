...
class DeepseekV2MoE(nn.Module):
    ...
    def forward_deepep(
        ...
        # 记录dispatch前的状态
        logger.info(f"🔧 [forward_deepep] before dispatch - hidden_states.shape={hidden_states.shape}, dtype={hidden_states.dtype}")
        logger.info(f"🔧 [forward_deepep] before dispatch - topk_idx.shape={topk_idx.shape}, dtype={topk_idx.dtype}")
        logger.info(f"🔧 [forward_deepep] before dispatch - topk_weights.shape={topk_weights.shape}, dtype={topk_weights.dtype}")
        logger.info(f"🔧 [forward_deepep] forward_batch.is_extend_in_batch={forward_batch.is_extend_in_batch}")
        if self.ep_size > 1:
            ...
            # 记录dispatch后的状态
            logger.info(f"🔧 [forward_deepep] after dispatch - hidden_states.shape={hidden_states.shape}, dtype={hidden_states.dtype}")
            logger.info(f"🔧 [forward_deepep] after dispatch - topk_idx.shape={topk_idx.shape}, dtype={topk_idx.dtype}")
            logger.info(f"🔧 [forward_deepep] after dispatch - topk_weights.shape={topk_weights.shape}, dtype={topk_weights.dtype}")
            if reorder_topk_ids is not None:
                logger.info(f"🔧 [forward_deepep] after dispatch - reorder_topk_ids.shape={reorder_topk_ids.shape}, dtype={reorder_topk_ids.dtype}")
            if num_recv_tokens_per_expert is not None:
                logger.info(f"🔧 [forward_deepep] after dispatch - num_recv_tokens_per_expert={num_recv_tokens_per_expert}")
            if seg_indptr is not None:
                logger.info(f"🔧 [forward_deepep] after dispatch - seg_indptr.shape={seg_indptr.shape}, dtype={seg_indptr.dtype}")
            if masked_m is not None:
                logger.info(f"🔧 [forward_deepep] after dispatch - masked_m={masked_m}")
            if expected_m is not None:
                logger.info(f"🔧 [forward_deepep] after dispatch - expected_m={expected_m}")
        ...
        logger.info(f"🔧 [forward_deepep] final_hidden_states.shape={final_hidden_states.shape}, dtype={final_hidden_states.dtype}")
...
class DeepseekV2Model(nn.Module):
    ...
    def forward(
        ...
        log_info_on_rank0(logger, f"🔧 [DEEPSEEK_V2_MODEL] Forward batch can run TBO: {forward_batch.can_run_tbo}")
...
class DeepseekV2ForCausalLM(nn.Module):
    ...
    def load_weights(self, weights: Iterable[Tuple[str, torch.Tensor]], is_nextn=False):
        ...
        # log_info_on_rank0(logger, f"🔧 [DEEPSEEK_V2] Stacked params mapping created, stacked_params_mapping={stacked_params_mapping}")
        ...
        # log_info_on_rank0(logger, f"🔧 [DEEPSEEK_V2] params_dict.keys(): {params_dict.keys()}")
        weight_names = []
        for name, loaded_weight in weights:
            # log_info_on_rank0(logger, f"🔧 [DEEPSEEK_V2] Loading weight: {name}, shape: {loaded_weight.shape}, dtype: {loaded_weight.dtype}")
            ...
            for param_name, weight_name, shard_id in stacked_params_mapping:
                ...
                # log_info_on_rank0(logger, f"🔧 [DEEPSEEK_V2] [STACKED] Loading stacked param: {name}, shape: {param.shape}, dtype: {param.dtype}, weight_loader: {weight_loader.__name__ if hasattr(weight_loader, '__name__') else type(weight_loader).__name__}")
                weight_loader(param, loaded_weight, shard_id)
                # log_info_on_rank0(logger, f"🔧 [DEEPSEEK_V2] [STACKED] Successfully loaded stacked param: {name}")
                break
            else:
                for mapping in expert_params_mapping:
                    ...
                    param = params_dict[name]
                    weight_loader = param.weight_loader
                    # log_info_on_rank0(logger, f"🔧 [DEEPSEEK_V2] [EXPERT] Loading expert param: {name}, expert_id: {expert_id}, shape: {param.shape}, dtype: {param.dtype}, weight_loader: {weight_loader.__name__ if hasattr(weight_loader, '__name__') else type(weight_loader).__name__}")
                    weight_loader(
                        param,
                        loaded_weight,
                        name,
                    ...
                    # log_info_on_rank0(logger, f"🔧 [DEEPSEEK_V2] [EXPERT] Successfully loaded expert param: {name}, expert_id: {expert_id}")
                    break
                else:
                    # Skip loading extra bias for GPTQ models.
                    if name.endswith(".bias") and name not in params_dict:
                    ...
                    if fuse_qkv_a_proj and (
                        ...
                        if (
                            ...
                            # log_info_on_rank0(logger, f"🔧 [DEEPSEEK_V2] [FUSED_QKV] Loading fused QKV param: {param_name}, shape: {param.shape}, dtype: {param.dtype}, cat_dim: {cat_dim}, weight_loader: {weight_loader.__name__ if hasattr(weight_loader, '__name__') else type(weight_loader).__name__}")
                            weight_loader(param, fused_weight)
                            # log_info_on_rank0(logger, f"🔧 [DEEPSEEK_V2] [FUSED_QKV] Successfully loaded fused QKV param: {param_name}")
                        ...
                        # log_info_on_rank0(logger, f"🔧 [DEEPSEEK_V2] [DEFAULT] Loading default param: {name}, shape: {param.shape}, dtype: {param.dtype}, weight_loader: {weight_loader.__name__ if hasattr(weight_loader, '__name__') else type(weight_loader).__name__}")
                        weight_loader(param, loaded_weight)
                        # log_info_on_rank0(logger, f"🔧 [DEEPSEEK_V2] [DEFAULT] Successfully loaded default param: {name}")

        self.post_load_weights(is_nextn=is_nextn, weight_names=weight_names)
        # log_info_on_rank0(logger, f"🔧 [DEEPSEEK_V2] [COMPLETED] Weights loading completed")
